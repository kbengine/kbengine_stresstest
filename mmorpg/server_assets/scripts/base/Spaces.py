...
class Spaces(KBEngine.Entity, GameObject):
	...
	def loginToSpace(self, avatarEntity, spaceUType, context):
		...
		某个玩家请求登陆到某个space中
		"""
		# 如果ID等于这个，需要将avatarEntity均衡分布到cell_scene中
		if spaceUType == 1000:
			count = 99999999999

			for idx in range(GlobalConst.MAX_STRESSTEST_CELL_SPACE - 1):
				v = self.spacesPlayerCount.get(1000 + idx + 1, 0)
				if v < count:
					spaceUType = 1000 + idx + 1
					count = v

		if spaceUType in self.spacesPlayerCount:
			self.spacesPlayerCount[spaceUType] += 1
		else:
			self.spacesPlayerCount[spaceUType] = 1
		
		#print("--------enter---------------------------%s" % str(self.spacesPlayerCount[spaceUType]))
	...
	def logoutSpace(self, avatarID, spaceID):
		...
		for spaceAlloc in self._spaceAllocs.values():
			for spaceMB in spaceAlloc.getSpaces().values():
				if spaceMB and spaceMB.id == spaceID:
					spaceMB.logoutSpace(avatarID)
					if spaceAlloc.getUType() in self.spacesPlayerCount:

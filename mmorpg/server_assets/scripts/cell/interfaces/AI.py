...
class AI:
	...
	def checkInTerritory(self):
		...
		if not ret:
			INFO_MSG("%s::checkInTerritory: %i is False." % (self.className, self.id))
	...
	def addTerritory(self):
		...
		if self.territoryControllerID <= 0:
			ERROR_MSG("%s::addTerritory: %i, range=%i, is error!" % (self.className, self.id, trange))
		else:
			INFO_MSG("%s::addTerritory: %i range=%i, id=%i." % (self.className, self.id, trange, self.territoryControllerID))
	...
	def delTerritory(self):
		...
		if self.territoryControllerID > 0:
			...
			INFO_MSG("%s::delTerritory: %i" % (self.className, self.id))
	...
	def onWitnessed(self, isWitnessed):
		...
		"""
		#INFO_MSG("%s::onWitnessed: %i isWitnessed=%i." % (self.className, self.id, isWitnessed))
	...
	def onEnterTrap(self, entityEntering, range_xz, range_y, controllerID, userarg):
		...
		if entityEntering.isDestroyed or not entityEntering.isPlayer() or entityEntering.isDead():
			return
	...
	def onLeaveTrap(self, entityLeaving, range_xz, range_y, controllerID, userarg):
		...
		if entityLeaving.isDestroyed or not entityLeaving.isPlayer() or entityLeaving.isDead():
			return
	...
	def onLoseTarget(self):
		...
		敌人丢失
		"""
		INFO_MSG("%s::onLoseTarget: %i target=%i, enemyLogSize=%i." % (self.className, self.id, \
	...
	def onEnemyEmpty(self):
		...
		敌人列表空了
		"""
		INFO_MSG("%s::onEnemyEmpty: %i" % (self.className, self.id))
